...
public class CrtGridScanlineOp implements BufferedImageOp {

  private final float scanlineDarkness; // Dominant effect (e.g. 0.40f)
  private final float pixelGapDarkness; // Subtle effect (e.g. 0.15f)
  private final int bloomThreshold;     // Light cutoff (0-255, e.g. 180)
  private final float bloomBoost;       // Boost intensity (e.g. 0.25f)

  public CrtGridScanlineOp(float scanlineDarkness, float pixelGapDarkness, int bloomThreshold, float bloomBoost) {
    this.scanlineDarkness = Math.max(0.0f, Math.min(1.0f, scanlineDarkness));
    this.pixelGapDarkness = Math.max(0.0f, Math.min(1.0f, pixelGapDarkness));
    this.bloomThreshold = bloomThreshold;
    this.bloomBoost = Math.max(0.0f, bloomBoost);
  }

  public CrtGridScanlineOp() {
    this(0.40f, 0.15f, 180, 0.25f);
  }
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dest) {
    ...
    for (int y = 0; y < srcH; y++) {
      for (int x = 0; x < srcW; x++) {
        ...
        // --- Calculate Pixel Variant Colors ---
        // Base pixel (cols 0 and 1)
        int basePixel = (a << 24) | (r << 16) | (g << 8) | b;

        // Column gap pixel (subtle vertical separator on col 2)
        float colGapFactor = 1.0f - pixelGapDarkness;
        int gr = (int) (r * colGapFactor);
        int gg = (int) (g * colGapFactor);
        int gb = (int) (b * colGapFactor);
        int colGapPixel = (a << 24) | (gr << 16) | (gg << 8) | gb;

        // Scanline row pixel (dominant horizontal bar on row 2)
        ...
        // Intersection of scanline and vertical gap (row 2, col 2)
        int sgr = (int) (sr * colGapFactor);
        int sgg = (int) (sg * colGapFactor);
        int sgb = (int) (sb * colGapFactor);
        int scanlineGapPixel = (a << 24) | (sgr << 16) | (sgg << 8) | sgb;

        // --- Map to 3x3 Block ---
        int outX = x * 3;
        int outY = y * 3;

        // Rows 0 & 1: Base pixel with slight vertical gap at column 2
        for (int subY = 0; subY < 2; subY++) {
          int rowOffset = (outY + subY) * dstW + outX;
          dstData[rowOffset]     = basePixel;
          dstData[rowOffset + 1] = basePixel;
          dstData[rowOffset + 2] = colGapPixel;
        }

        // Row 2: Scanline row with intersection pixel at column 2
        int scanOffset = (outY + 2) * dstW + outX;
        dstData[scanOffset]     = scanlinePixel;
        dstData[scanOffset + 1] = scanlinePixel;
        dstData[scanOffset + 2] = scanlineGapPixel;
      }
    }
